...
class LogsGathering():
	...
	def prepare_places(self):
		...
		'''
		if not logs_folder in os.listdir(place_for_logs_folder):
			os.mkdir(logs_folder_path)
		log_folders = os.listdir(logs_folder_path)
		for k, v in logs_gathering['file_logs'].items():
			if not k in log_folders:
				save_path = os.path.join(logs_folder_path, k)
				os.mkdir(save_path)
				save_path = os.path.join(logs_folder_path, k, 'history')
				os.mkdir(save_path)
		for k, v in logs_gathering['command_logs'].items():
			if not k in log_folders:
				save_path = os.path.join(logs_folder_path, k)
				os.mkdir(save_path)
				save_path = os.path.join(logs_folder_path, k, 'history')
	...
	def search_add_new_logs(ask=True):
		'''
		Searches for log files.

		Adds them to logs_gathering config file.
	...
	def quick_logs_gatherer(self):
		...
		for name, log in self.logs_gathering['command_logs'].items():
			file_name = date()
			save_as = os.path.join(logs_folder_path, 'history', file_name)
			command = '{} > {}'.format(log['command'], save_as)
			os.system(command)

		for name, log in self.logs_gathering['file_logs'].items():
			file_name = date()
			save_as = os.path.join(logs_folder_path, name, 'history', file_name)
			command = 'cat {} > {}'.format(log['path'], save_as)
			os.system(command)
	...
	def ongoing_bash(self, every_n_seconds=1, script_path, arg_str=''):
		'''
		.
		'''
